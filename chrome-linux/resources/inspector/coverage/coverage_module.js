Coverage.RangeUseCount;Coverage.CoverageSegment;Coverage.CoverageType={CSS:(1<<0),JavaScript:(1<<1),JavaScriptCoarse:(1<<2),};Coverage.SuspensionState={Active:Symbol('Active'),Suspending:Symbol('Suspending'),Suspended:Symbol('Suspended')};Coverage.CoverageModel=class extends SDK.SDKModel{constructor(target){super(target);this._cpuProfilerModel=target.model(SDK.CPUProfilerModel);this._cssModel=target.model(SDK.CSSModel);this._debuggerModel=target.model(SDK.DebuggerModel);this._coverageByURL=new Map();this._coverageByContentProvider=new Map();this._bestEffortCoveragePromise=null;this._suspensionState=Coverage.SuspensionState.Active;this._pollTimer=null;this._currentPollPromise=null;this._shouldResumePollingOnResume=false;}
async start(){if(this._suspensionState!==Coverage.SuspensionState.Active)
throw Error('Cannot start CoverageModel while it is not active.');const promises=[];if(this._cssModel){this._clearCSS();promises.push(this._cssModel.startCoverage());}
...
async stop(){await this.stopPolling();const promises=[];if(this._cpuProfilerModel)
promises.push(this._cpuProfilerModel.stopPreciseCoverage());if(this._cssModel)
promises.push(this._cssModel.stopCoverage());await Promise.all(promises);}
reset(){this._coverageByURL=new Map();this._coverageByContentProvider=new Map();}
async startPolling(){if(this._currentPollPromise||this._suspensionState!==Coverage.SuspensionState.Active)
return;await this._pollLoop();}
async _pollLoop(){const coveragePollingPeriodMs=700;this._clearTimer();this._currentPollPromise=this._pollAndCallback();await this._currentPollPromise;if(this._suspensionState===Coverage.SuspensionState.Active)
...
async _pollAndCallback(){if(this._suspensionState===Coverage.SuspensionState.Suspended)
return;const updates=await this._takeAllCoverage();console.assert(this._suspensionState!==Coverage.SuspensionState.Suspended,'CoverageModel was suspended while polling.');if(updates.length)
...
async preSuspendModel(){if(this._suspensionState!==Coverage.SuspensionState.Active)
return;this._suspensionState=Coverage.SuspensionState.Suspending;if(this._currentPollPromise){await this.stopPolling();this._shouldResumePollingOnResume=true;}}
async suspendModel(){this._suspensionState=Coverage.SuspensionState.Suspended;}
async resumeModel(){this._suspensionState=Coverage.SuspensionState.Active;}
async postResumeModel(){if(this._shouldResumePollingOnResume){await this.startPolling();this._shouldResumePollingOnResume=false;}}
...
const coverageByUrlKeys=Array.from(this._coverageByURL.keys()).sort();for(const urlInfoKey of coverageByUrlKeys){const urlInfo=this._coverageByURL.get(urlInfoKey);const url=urlInfo.url();if(url.startsWith('extensions::')||url.startsWith('chrome-extension://'))
...
for(const infoKey of coverageByLocationKeys){const info=urlInfo._coverageInfoByLocation.get(infoKey);const entry={url,ranges:[],text:await info.contentProvider().requestContent()};let start=0;for(const segment of info._segments){if(segment.count)
entry.ranges.push({start:start,end:segment.end});else
start=segment.end;}
result.push(entry);}}
await fos.write(JSON.stringify(result,undefined,2));fos.close();}};SDK.SDKModel.register(Coverage.CoverageModel,SDK.Target.Capability.None,false);Coverage.CoverageModel.Events={CoverageUpdated:Symbol('CoverageUpdated')};Coverage.URLCoverageInfo=class{constructor(url){this._url=url;this._coverageInfoByLocation=new Map();this._size=0;this._usedSize=0;this._type;this._isContentScript=false;}
...
_clear(){if(this._model)
this._model.reset();this._reset();}
...
this.stopRecording();}
async ensureRecordingStarted(){const enable=!this._toggleRecordAction.toggled();if(enable)
...
this._model=mainTarget.model(Coverage.CoverageModel);Host.userMetrics.actionTaken(Host.UserMetrics.Action.CoverageStarted);const success=await this._model.start();if(!success)
return;this._model.addEventListener(Coverage.CoverageModel.Events.CoverageUpdated,this._onCoverageDataReceived,this);this._resourceTreeModel=(mainTarget.model(SDK.ResourceTreeModel));if(this._resourceTreeModel){this._resourceTreeModel.addEventListener(SDK.ResourceTreeModel.Events.MainFrameNavigated,this._onMainFrameNavigated,this);}
...
await this._model.stop();this._model.removeEventListener(Coverage.CoverageModel.Events.CoverageUpdated,this._onCoverageDataReceived,this);this._toggleRecordAction.setToggled(false);if(this._startWithReloadButton)
...
return;this._model.exportReport(fos);}
...
_innerHandleAction(coverageView,actionId){switch(actionId){case'coverage.toggle-recording':coverageView._toggleRecording();break;case'coverage.start-with-reload':coverageView._startRecording(true);break;default:console.assert(false,`Unknown action: ${actionId}`);}}};;Coverage.RawLocation;Coverage.CoverageDecorationManager=class{constructor(coverageModel){this._coverageModel=coverageModel;this._textByProvider=new Map();this._uiSourceCodeByContentProvider=new Multimap();for(const uiSourceCode of Workspace.workspace.uiSourceCodes())
...
async usageByLine(uiSourceCode){const result=[];const content=await uiSourceCode.requestContent();if(!content)
return[];const sourceText=new TextUtils.Text((content));await this._updateTexts(uiSourceCode,sourceText);const lineEndings=sourceText.lineEndings();for(let line=0;line<sourceText.lineCount();++line){const lineLength=lineEndings[line]-(line?lineEndings[line-1]:0)-1;if(!lineLength){result.push(undefined);continue;}
...
if(contentType.isStyleSheet()||contentType.isDocument()){const rawStyleLocations=Bindings.cssWorkspaceBinding.uiLocationToRawLocations(new Workspace.UILocation(uiSourceCode,line,column));for(const location of rawStyleLocations){const header=location.header();if(!header)
...
_innerDecorate(uiSourceCode,textEditor,lineUsage){const gutterType=Coverage.CoverageView.LineDecorator._gutterType;this._uninstallGutter(textEditor);if(lineUsage.length)
this._installGutter(textEditor,uiSourceCode.url());for(let line=0;line<lineUsage.length;++line){if(typeof lineUsage[line]!=='boolean')
continue;const className=lineUsage[line]?'text-editor-coverage-used-marker':'text-editor-coverage-unused-marker';const gutterElement=createElementWithClass('div',className);textEditor.setGutterDecoration(line,gutterType,gutterElement);}}
makeGutterClickHandler(url){function handleGutterClick(event){const eventData=(event.data);if(eventData.gutterType!==Coverage.CoverageView.LineDecorator._gutterType)
return;const coverageViewId='coverage';UI.viewManager.showView(coverageViewId).then(()=>UI.viewManager.view(coverageViewId).widget()).then(widget=>{const matchFormattedSuffix=url.match(/(.*):formatted$/);const urlWithoutFormattedSuffix=(matchFormattedSuffix&&matchFormattedSuffix[1])||url;widget.selectCoverageItemByUrl(urlWithoutFormattedSuffix);});}
